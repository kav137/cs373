...
class robot:
    ...
    def move(self, motion):
        test = robot()
        test.set(self.x, self.y, self.orientation % (2 * pi))
        test.set_noise(self.bearing_noise, self.steering_noise,
                       self.distance_noise)

        steering_angle = random.gauss(motion[0], test.steering_noise)
        distance = random.gauss(motion[1], test.distance_noise)

        if steering_angle < 0.001:
            self.x += distance * cos(test.orientation)
            self.y += distance * sin(test.orientation)
            return self

        turn_radius = test.length / tan(steering_angle)
        turn_angle = distance / turn_radius

        turn_point_x = test.x - sin(test.orientation) * turn_radius
        turn_point_y = test.y + cos(test.orientation) * turn_radius

        test.orientation += turn_angle
        test.x = turn_point_x + sin(test.orientation) * turn_radius
        test.y = turn_point_y - cos(test.orientation) * turn_radius
        return test
    #
    ...
    # --------
    def sense(self, apply_noise=1):
        z = []
        for i in range(len(landmarks)):
            mark_y, mark_x = landmarks[i]
            measurement = atan2(mark_y - self.y, mark_x -
...
# print 'Robot:        ', myrobot
# print 'Measurements: ', myrobot.sense(0)
##
